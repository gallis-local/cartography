# Fork image: installs cartography from local source instead of PyPI.
# Used for building images from forks or branches before publishing.
FROM python:3.10.20-slim@sha256:87579103010e46d38cae5e8d6c979f2cdbc9ef753daf603813c74cc7e995f6a7 AS base
...
# Install cartography from local source
COPY --chown=${uid}:${gid} . /var/cartography
RUN ls -alh /var/cartography
RUN uv tool install /var/cartography
RUN ls -alh /var/cartography
