...
async def get(controller: Controller) -> list[dict[str, Any]]:
    """
    Retrieve all UniFi clients (including historical/disconnected) from the controller.

    :param controller: Controller instance
    :return: List of client data
    """
    logger.debug("Fetching all UniFi clients (historical)")
    await controller.clients_all.update()

    # Convert aiounifi Client objects to dictionaries
    clients = []
    for client in controller.clients_all.values():
        ...
        clients.append(
            {
                ...
                # Mark as historical since this comes from clients_all endpoint
                "is_historical": True,
    ...
    logger.debug("Fetched %d UniFi clients (historical)", len(clients))
    return clients


@timeit
def load_clients_all(
    ...
    """
    Load UniFi historical clients into Neo4j.
    ...
    """
    logger.debug("Loading %d UniFi historical clients to the graph.", len(data))
    load(
        neo4j_session,
        UnifiClientSchema(),
        data,
...
def cleanup(
    ...
    """
    Clean up stale UniFi historical clients from Neo4j.

    Note: This uses the same cleanup as regular clients since they share the same node label.
    The cleanup is handled by the regular clients sync.

    :param neo4j_session: Neo4j session
    :param common_job_parameters: Common job parameters
    """
    logger.debug("UniFi historical clients cleanup handled by regular clients sync")
...
async def sync(
    ...
    load_clients_all(
        neo4j_session, clients, site_id, common_job_parameters["UPDATE_TAG"]
    )
    cleanup(neo4j_session, common_job_parameters)

    stat_handler.incr("unifi_clients_all_synced", len(clients))
