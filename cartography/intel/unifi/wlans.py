        ...
        wlans.append(
            {
                "_id": wlan.id,
                "name": wlan.name,
                "enabled": getattr(wlan, "enabled", True),
                "is_guest": getattr(wlan, "is_guest", False),
                "security": getattr(wlan, "security", None),
                "wpa_mode": getattr(wlan, "wpa_mode", None),
                "wpa_enc": getattr(wlan, "wpa_enc", None),
                "usergroup_id": getattr(wlan, "usergroup_id", None),
                "hide_ssid": getattr(wlan, "hide_ssid", False),
                "mac_filter_enabled": getattr(wlan, "mac_filter_enabled", False),
                "site_id": site_id,
            }
        )
    return wlans
