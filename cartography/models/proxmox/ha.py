...
class ProxmoxHAResourceToVMMatchLinkProperties(CartographyRelProperties):
    """
    Properties for HA resource to VM PROTECTS relationship.
    """
    ...
    source_node_matcher: SourceNodeMatcher = make_source_node_matcher(
        {
            "id": PropertyRef(
                "ha_resource_id"
            ),  # Full cluster-scoped ID (cluster_id/ha/resource/sid)
        }
    )
    direction: LinkDirection = LinkDirection.OUTWARD
    rel_label: str = "PROTECTS"
    properties: ProxmoxHAResourceToVMMatchLinkProperties = (
        ProxmoxHAResourceToVMMatchLinkProperties()
    )
