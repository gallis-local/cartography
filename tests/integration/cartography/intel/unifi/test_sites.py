...
async def test_cleanup_unifi_sites_scoped_to_host(mock_get, neo4j_session):
    ...
    # Sync east controller's site: a different site, later UPDATE_TAG, simulating
    # the two cronjobs running independently. A controller's site id is a Mongo
    # ObjectId generated by that controller, so two controllers never share one --
    # both of a real deployment's sites are *named* "default" while carrying
    # distinct ids. Reusing one id here would instead test whether two controllers
    # collapse onto a single node, which is a question about node identity, not
    # about whether cleanup stays scoped to its own controller.
    mock_get.return_value = [
        {
            "id": "63301044fb46230763e5ed43",
            "name": "Default",
            "desc": "Default Site",
            "role": "admin",
        },
