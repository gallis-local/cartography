...
@pytest.mark.asyncio
@patch.object(cartography.intel.unifi.firewall_zones, "get", new_callable=AsyncMock, return_value=UNIFI_FIREWALL_ZONES)
async def test_sync_firewall_zones(mock_get, neo4j_session):
    ...
    assert check_nodes(neo4j_session, "UnifiFirewallZone", ["id", "name"]) == expected_nodes
...
async def test_sync_firewall_zones_cleanup(mock_get, neo4j_session):
    ...
    # Create a stale firewall zone
    neo4j_session.run(
        """
        MERGE (z:UnifiFirewallZone{id: 'stale_zone'})
        ON CREATE SET z.firstseen = timestamp()
        SET z.name = 'Stale Zone', z.lastupdated = $old_update_tag
        WITH z
        MATCH (s:UnifiSite{id: $site_id})
        MERGE (s)<-[r:RESOURCE]-(z)
